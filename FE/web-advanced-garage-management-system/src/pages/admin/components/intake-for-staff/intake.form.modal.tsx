...
const IntakeFormModal = ({ open, mode, intakeId, detailData, onClose, onSuccess }: Props) => {
  ...
  useEffect(() => {
    if (!selectedCustomerId) { setCustomerCars([]); return; }
    setCarsLoading(true);
  ...
  useEffect(() => {
    ...
    if (mode === "create") {
      ...
      form.setFieldsValue({
        ...
        vehicleConditions: [{
          frontStatus: "CLEAN", rearStatus: "CLEAN",
          leftStatus: "CLEAN", rightStatus: "CLEAN",
          roofStatus: "CLEAN", conditionNote: "",
        }],
      });
      return;
    }
    ...
    if (mode === "update" && detailData) {
      form.setFieldsValue({
        maintenance: {
          maintenanceType: detailData.maintenanceStatus,
          notes: "",
        ...
        customerGender: detailData.customer.gender ?? "",
    ...
    try {
      ...
      if (mode === "create") {
        ...
        await updateIntake(intakeId, {
          ...
          serviceDetails: v.serviceDetails ?? [],
          partDetails: v.partDetails ?? [],
          vehicleCondition: (v.vehicleConditions ?? []).map((c: {
            frontStatus: string; rearStatus: string; leftStatus: string;
            rightStatus: string; roofStatus: string; conditionNote: string;
          }) => ({
            frontStatus: c.frontStatus,
            rearStatus: c.rearStatus,
            leftStatus: c.leftStatus,
  ...
  return (
    <Modal
      ...
      cancelText={t("cancel")}
      styles={{ body: { maxHeight: "72vh", overflowY: "auto", padding: "1.25rem 1.5rem" } }}
    >
      <FormWrapper>
        <ConfigProvider theme={{
          token: { colorText: "#000000", colorTextPlaceholder: "#000000", colorBgContainer: "#ffffff" },
          components: {
        ...
        <Form form={form} layout="vertical" requiredMark={false}>
          ...
          <Row gutter={16}>
            <Col span={8}>
              <Form.Item name={["maintenance", "maintenanceType"]} label={t("intakeColType")} rules={[{ required: true }]}>
                <Select
                  options={[
            ...
            <Col span={8}>
              <Form.Item name={["maintenance", "assignedTechnicianId"]} label={t("intakeFormTechnicianId")} rules={[{ required: true }]}>
                <Select
                  showSearch
                  filterOption={(input, opt) => (opt?.label ?? "").toString().toLowerCase().includes(input.toLowerCase())}
                  options={technicians.map((t) => ({ value: t.technicianId, label: t.fullName }))}
                  classNames={{ popup: { root: "intake-form-dropdown" } }}
                  style={{ color: "#000000" }}
                  labelRender={BLACK_LABEL_RENDER}
                />
            ...
            <Col span={8}>
              <Form.Item name={["maintenance", "bayId"]} label={t("intakeFormBayId")}>
                <InputNumber style={{ width: "100%" }} min={1} />
              </Form.Item>
            </Col>
          </Row>
          <Form.Item name={["maintenance", "notes"]} label={t("intakeDetailNotes")}>
            <TextArea rows={2} />
          </Form.Item>
          ...
          {isCreate && customerMode === "existing" && (
            <Form.Item name="customerId" label={t("intakeFormCustomerId")} rules={[{ required: true }]}>
              <Select
                showSearch
          ...
          {(!isCreate || customerMode === "new") && (
            <Row gutter={16}>
              <Col span={8}>
                <Form.Item name="customerFullName" label={t("intakeDetailFullName")}>
                  <Input />
                </Form.Item>
              </Col>
              <Col span={8}>
                <Form.Item name="customerPhone" label={t("intakeDetailPhone")}>
                  <Input />
                </Form.Item>
              </Col>
              <Col span={8}>
                <Form.Item name="customerEmail" label={t("intakeDetailEmail")}>
                  <Input />
                </Form.Item>
              </Col>
              {!isCreate && (
                <>
                  <Col span={8}>
                    <Form.Item name="customerGender" label={t("intakeFormGender")}>
                      <Select
                        allowClear
                        options={[
                  ...
                  <Col span={8}>
                    <Form.Item name="customerDob" label={t("intakeDetailDob")}>
          ...
          {isCreate && carMode === "existing" && (
            <Form.Item name="carId" label={t("intakeFormCarId")} rules={[{ required: true }]}>
              <Select
                loading={carsLoading}
                disabled={!selectedCustomerId}
                placeholder={selectedCustomerId ? t("intakeFormSelectCar") : t("intakeFormSelectCustomerFirst")}
          ...
          {(!isCreate || carMode === "new") && (
            <Row gutter={16}>
              <Col span={8}>
                <Form.Item name="carLicensePlate" label={t("intakeDetailLicensePlate")}>
                  <Input />
                </Form.Item>
              </Col>
              <Col span={8}>
              ...
              <Col span={8}>
                <Form.Item name="carOdometer" label={t("intakeDetailOdometer")}>
                  <InputNumber style={{ width: "100%" }} min={0} />
                </Form.Item>
              </Col>
              <Col span={8}>
                <Form.Item name="carEngineNumber" label={t("intakeDetailEngine")}>
                  <Input />
                </Form.Item>
              </Col>
              <Col span={8}>
                <Form.Item name="carChassisNumber" label={t("intakeFormChassis")}>
                  <Input />
                </Form.Item>
              </Col>
            </Row>
              ...
              <>
                {fields.map(({ key, name }) => (
                  <Row key={key} gutter={8} align="middle">
                    <Col span={7}>
                      <Form.Item name={[name, "productId"]} label={t("intakeFormProductId")} rules={[{ required: true }]}>
                        <Select
                          showSearch
                          filterOption={(input, opt) => (opt?.label ?? "").toString().toLowerCase().includes(input.toLowerCase())}
                    ...
                    <Col span={5}>
                      <Form.Item name={[name, "quantity"]} label={t("intakeDetailQty")} rules={[{ required: true }]}>
                        <InputNumber style={{ width: "100%" }} min={1} />
                      </Form.Item>
                    </Col>
                    <Col span={10}>
                      <Form.Item name={[name, "notes"]} label={t("intakeDetailNotes")}>
                        <Input />
                      </Form.Item>
                    </Col>
                ...
                <Button type="dashed" icon={<PlusOutlined />} onClick={() => add({ quantity: 1, notes: "" })} block>
              ...
              <>
                {fields.map(({ key, name }) => (
                  <ConditionCard key={key}>
                    <Row gutter={12}>
                      <Col span={8}>
                        <Form.Item name={[name, "frontStatus"]} label={t("intakeDetailFront")}>
                          <Select options={CONDITION_OPTIONS} classNames={{ popup: { root: "intake-form-dropdown" } }} style={{ color: "#000000" }} labelRender={BLACK_LABEL_RENDER} />
                        </Form.Item>
                      </Col>
                      <Col span={8}>
                        <Form.Item name={[name, "rearStatus"]} label={t("intakeDetailRear")}>
                          <Select options={CONDITION_OPTIONS} classNames={{ popup: { root: "intake-form-dropdown" } }} style={{ color: "#000000" }} labelRender={BLACK_LABEL_RENDER} />
                        </Form.Item>
                      </Col>
                      <Col span={8}>
                        <Form.Item name={[name, "roofStatus"]} label={t("intakeDetailRoof")}>
                          <Select options={CONDITION_OPTIONS} classNames={{ popup: { root: "intake-form-dropdown" } }} style={{ color: "#000000" }} labelRender={BLACK_LABEL_RENDER} />
                        </Form.Item>
                      </Col>
                      <Col span={8}>
                        <Form.Item name={[name, "leftStatus"]} label={t("intakeDetailLeft")}>
                          <Select options={CONDITION_OPTIONS} classNames={{ popup: { root: "intake-form-dropdown" } }} style={{ color: "#000000" }} labelRender={BLACK_LABEL_RENDER} />
                        </Form.Item>
                      </Col>
                      <Col span={8}>
                        <Form.Item name={[name, "rightStatus"]} label={t("intakeDetailRight")}>
                          <Select options={CONDITION_OPTIONS} classNames={{ popup: { root: "intake-form-dropdown" } }} style={{ color: "#000000" }} labelRender={BLACK_LABEL_RENDER} />
                        </Form.Item>
                      </Col>
                      <Col span={8}>
                        <Form.Item name={[name, "conditionNote"]} label={t("intakeDetailConditionNote")}>
                          <Input />
                        </Form.Item>
                      </Col>
                    </Row>
                    <RemoveBtn type="text" danger icon={<DeleteOutlined />} onClick={() => remove(name)}>
                      {t("intakeFormRemoveCondition")}
                    </RemoveBtn>
                  </ConditionCard>
                ))}
                <Button type="dashed" icon={<PlusOutlined />} onClick={() => add({
                  frontStatus: "CLEAN", rearStatus: "CLEAN", leftStatus: "CLEAN",
                  rightStatus: "CLEAN", roofStatus: "CLEAN", conditionNote: "",
                })} block>
