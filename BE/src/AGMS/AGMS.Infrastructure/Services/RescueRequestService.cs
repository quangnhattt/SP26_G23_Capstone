...
public class RescueRequestService : IRescueRequestService
{
    ...
    /// </summary>
    public async Task<RescueRequestDetailDto> CreateAsync(int customerId, CreateRescueRequestDto request, CancellationToken ct)
    {
        var car = await _rescueRepo.GetCarByIdAsync(request.CarId, ct)
            ?? throw new KeyNotFoundException("Xe không tồn tại.");

        var customer = await _userRepo.GetByIdAsync(customerId, ct)
    ...
    /// </summary>
    public async Task<RescueRequestDetailDto> ProposeAsync(int rescueId, int saId, ProposeRescueDto request, CancellationToken ct)
    {
        var rescue = await _rescueRepo.GetByIdAsync(rescueId, ct)
            ?? throw new KeyNotFoundException($"Yêu cầu cứu hộ ID={rescueId} không tồn tại.");
        ...
        var newStatus = request.RescueType.ToUpperInvariant() == RescueType.Roadside
            ? RescueStatus.ProposedRoadside : RescueStatus.ProposedTowing;
    ...
    /// </summary>
    public async Task<RescueRequestDetailDto> AssignTechnicianAsync(int rescueId, int saId, AssignTechnicianDto request, CancellationToken ct)
    {
        var rescue = await _rescueRepo.GetByIdAsync(rescueId, ct)
            ?? throw new KeyNotFoundException($"Yêu cầu cứu hộ ID={rescueId} không tồn tại.");
    ...
    /// </summary>
    public async Task<RescueRequestDetailDto> AcceptJobAsync(int rescueId, int techId, CancellationToken ct)
    {
        var rescue = await _rescueRepo.GetByIdAsync(rescueId, ct)
            ?? throw new KeyNotFoundException($"Yêu cầu cứu hộ ID={rescueId} không tồn tại.");
    ...
    /// </summary>
    public async Task<RescueRequestDetailDto> ArriveAsync(int rescueId, int techId, CancellationToken ct)
    {
        var rescue = await _rescueRepo.GetByIdAsync(rescueId, ct)
            ?? throw new KeyNotFoundException($"Yêu cầu cứu hộ ID={rescueId} không tồn tại.");
    ...
    /// </summary>
    public async Task<RescueRequestDetailDto> RecordConsentAsync(int rescueId, int actorId, CustomerConsentDto request, CancellationToken ct)
    {
        var rescue = await _rescueRepo.GetByIdAsync(rescueId, ct)
            ?? throw new KeyNotFoundException($"Yêu cầu cứu hộ ID={rescueId} không tồn tại.");
    ...
    /// </summary>
    public async Task<RescueRequestDetailDto> StartDiagnosisAsync(int rescueId, int techId, StartDiagnosisDto request, CancellationToken ct)
    {
        var rescue = await _rescueRepo.GetByIdAsync(rescueId, ct)
            ?? throw new KeyNotFoundException($"Yêu cầu cứu hộ ID={rescueId} không tồn tại.");
    ...
    public async Task<RepairItemsResultDto> AddRepairItemsAsync(int rescueId, AddRepairItemsDto request, CancellationToken ct)
    {
        ...
        {
            _ = await _rescueRepo.GetProductByIdAsync(item.ProductId, ct)
    ...
    /// </summary>
    public async Task<RescueRequestDetailDto> CompleteRepairAsync(int rescueId, int techId, CompleteRepairDto request, CancellationToken ct)
    {
        var rescue = await _rescueRepo.GetByIdAsync(rescueId, ct)
            ?? throw new KeyNotFoundException($"Yêu cầu cứu hộ ID={rescueId} không tồn tại.");
        ...
        if (rescue.ResultingMaintenanceID.HasValue && !string.IsNullOrWhiteSpace(request.CompletionNotes))
        {
            ...
            if (maintenance != null)
            {
                var existing = string.IsNullOrWhiteSpace(maintenance.Notes) ? "" : maintenance.Notes + "\n";
    ...
    /// </summary>
    public async Task<TowingDispatchResultDto> DispatchTowingAsync(int rescueId, int saId, DispatchTowingDto request, CancellationToken ct)
    {
        var rescue = await _rescueRepo.GetByIdAsync(rescueId, ct)
            ?? throw new KeyNotFoundException($"Yêu cầu cứu hộ ID={rescueId} không tồn tại.");
    ...
    /// </summary>
    public async Task<RescueRequestDetailDto> AcceptTowingAsync(int rescueId, int customerId, CancellationToken ct)
    {
        var rescue = await _rescueRepo.GetByIdAsync(rescueId, ct)
            ?? throw new KeyNotFoundException($"Yêu cầu cứu hộ ID={rescueId} không tồn tại.");
    ...
    /// </summary>
    public async Task<CompleteTowingResultDto> CompleteTowingAsync(int rescueId, int saId, CompleteTowingDto request, CancellationToken ct)
    {
        var rescue = await _rescueRepo.GetByIdAsync(rescueId, ct)
            ?? throw new KeyNotFoundException($"Yêu cầu cứu hộ ID={rescueId} không tồn tại.");
        ...
        var maintenance = new CarMaintenance
        {
            ...
            Notes                 = string.IsNullOrWhiteSpace(request.RepairOrderNotes)
                                        ? null : request.RepairOrderNotes.Trim(),
            CreatedBy             = saId,
            TotalAmount           = 0,
            DiscountAmount        = 0,
            MemberDiscountAmount  = 0,
    ...
    /// </summary>
    public async Task<CreateInvoiceResultDto> CreateInvoiceAsync(int rescueId, int saId, CreateInvoiceDto request, CancellationToken ct)
    {
        var rescue = await _rescueRepo.GetByIdAsync(rescueId, ct)
            ?? throw new KeyNotFoundException($"Yêu cầu cứu hộ ID={rescueId} không tồn tại.");
    ...
    /// </summary>
    public async Task<SendInvoiceResultDto> SendInvoiceAsync(int rescueId, int saId, CancellationToken ct)
    {
        var rescue = await _rescueRepo.GetByIdAsync(rescueId, ct)
            ?? throw new KeyNotFoundException($"Yêu cầu cứu hộ ID={rescueId} không tồn tại.");
    ...
    /// </summary>
    public async Task<AcceptInvoiceResultDto> AcceptInvoiceAsync(int rescueId, int customerId, CancellationToken ct)
    {
        var rescue = await _rescueRepo.GetByIdAsync(rescueId, ct)
            ?? throw new KeyNotFoundException($"Yêu cầu cứu hộ ID={rescueId} không tồn tại.");
    ...
    /// </summary>
    public async Task<PaymentResultDto> ProcessPaymentAsync(int rescueId, int customerId, ProcessPaymentDto request, CancellationToken ct)
    {
        var rescue = await _rescueRepo.GetByIdAsync(rescueId, ct)
            ?? throw new KeyNotFoundException($"Yêu cầu cứu hộ ID={rescueId} không tồn tại.");
        ...
        if (!rescue.ResultingMaintenanceID.HasValue)
            throw new InvalidOperationException("Rescue không có Repair Order. Không thể xử lý thanh toán.");
        ...
        if (Math.Round(request.Amount, 2) != Math.Round(finalAmount, 2))
            throw new ArgumentException($"Số tiền thanh toán không khớp với hóa đơn. Phải là {finalAmount:N0}đ.");
    ...
    /// </summary>
    public async Task<DisputeCreatedResultDto> CreateDisputeAsync(int rescueId, int customerId, CreateDisputeDto request, CancellationToken ct)
    {
        var rescue = await _rescueRepo.GetByIdAsync(rescueId, ct)
            ?? throw new KeyNotFoundException($"Yêu cầu cứu hộ ID={rescueId} không tồn tại.");
        ...
        if (rescue.ResultingMaintenanceID.HasValue)
        {
            ...
            {
                var existing = string.IsNullOrWhiteSpace(maintenance.Notes) ? "" : maintenance.Notes + "\n";
                maintenance.Notes = existing + $"[DISPUTE {now:yyyy-MM-dd HH:mm}] {request.Reason.Trim()}";
                await _rescueRepo.UpdateMaintenanceAsync(maintenance, ct);
            }
        }
    ...
    /// </summary>
    public async Task<ResolveDisputeResultDto> ResolveDisputeAsync(int rescueId, int saId, ResolveDisputeDto request, CancellationToken ct)
    {
        var rescue = await _rescueRepo.GetByIdAsync(rescueId, ct)
            ?? throw new KeyNotFoundException($"Yêu cầu cứu hộ ID={rescueId} không tồn tại.");
        ...
        if (!rescue.ResultingMaintenanceID.HasValue)
            throw new InvalidOperationException("Rescue không có Repair Order. Không thể xử lý tranh chấp.");
    ...
    /// </summary>
    public async Task<CancelRescueResultDto> CancelAsync(int rescueId, int saId, CancelRescueDto request, CancellationToken ct)
    {
        var rescue = await _rescueRepo.GetByIdAsync(rescueId, ct)
            ?? throw new KeyNotFoundException($"Yêu cầu cứu hộ ID={rescueId} không tồn tại.");
    ...
    /// </summary>
    public async Task<MarkSpamResultDto> MarkSpamAsync(int rescueId, int saId, MarkSpamDto request, CancellationToken ct)
    {
        var rescue = await _rescueRepo.GetByIdAsync(rescueId, ct)
            ?? throw new KeyNotFoundException($"Yêu cầu cứu hộ ID={rescueId} không tồn tại.");
